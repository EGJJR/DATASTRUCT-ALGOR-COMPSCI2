...
/*
 * The main() method is the entry point of the guessGame program.
 * 
 * It initializes the following parameters:
 * - input: A Scanner object to read input from the user
 * - number: A random integer between 1 and 1000 generated using Math.random()
 * - guess: An integer to hold the user's guess, initialized to 0
 * - count: An integer counter to track the number of guesses, initialized to 0 
 * - validInput: A boolean flag to indicate if the user input is valid, initialized to false
 * 
 * It prints an introductory message prompting the user to guess the number.
 * 
 * It then enters a while loop that continues until the user guesses the correct number.
 * 
 * Inside the loop:
 * - It prompts the user to enter a guess and reads it using input.nextInt()
 * - It uses a try/catch block to catch InputMismatchException if invalid input is entered
 * - If valid input, it increments count and checks if guess matches number
 * - If no match, it provides hints if guess is too high or too low
 * - If invalid input, it prints an error and discards the invalid input
 * 
 * After exiting the loop, the game ends when the user guesses the number correctly.
 * It prints a congratulatory message with the number of tries.
 */
